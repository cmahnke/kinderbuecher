...
function dotted(unit: Fraction, dots: number | undefined): Fraction {
  let frac = unit
  for (let i = 0; i < (dots ?? 0); i++) frac = frac.mul(3).div(2)
  return frac
}
...
class MnxToMusicXml {
  ...
  private startAttributes(
    ...
    for (const positioned of measure.clefs ?? []) {
  ...
  private staffCount(part: MnxPart, measure: MnxPartMeasure): number {
    ...
    for (const positioned of measure.clefs ?? []) max = Math.max(max, positioned.staff ?? 1)
